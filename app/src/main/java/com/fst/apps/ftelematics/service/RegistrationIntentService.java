...
public class RegistrationIntentService extends IntentService {
    ...
    private void sendRegistrationToServer(String token) {
        ...
        params.put("name", sharedPrefs.getUserId());
        params.put("email", sharedPrefs.getAccountId());
        params.put("imei", AppUtils.getDeviceIMEI(this));
        try {
           int status= networkUtility.sendPost(AppConstants.PUSH_SERVER_URL,params);
            if(status==200){
                sharedPrefs.setRegisteredForPush(true);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
